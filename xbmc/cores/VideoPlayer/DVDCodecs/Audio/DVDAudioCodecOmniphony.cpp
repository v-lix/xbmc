...
const char* CDVDAudioCodecOmniphony::CodecId(const CDVDStreamInfo& hints)
{
  switch (hints.codec)
  {
     ...
     * live and where the bridge decodes everything the stream carries. Auro-3D
     * is a presentation on top of MA as well - the same XLL asset, with the
     * height layer written into the low bits of its samples rather than
     * announced anywhere - so it belongs here for the same reason. That it
     * reports no object of its own is beside the point: its height channels
     * only exist to be recovered once the bridge has decoded the carrier.
     *
