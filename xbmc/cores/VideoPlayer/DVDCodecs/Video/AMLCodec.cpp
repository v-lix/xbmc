...
unsigned int CAMLCodec::GetDecoderVideoRate()
{
  ...
  // frame_dur is u32 in the kernel's vdec_info, so a decoder reporting a
  // negative duration arrives here as a number just short of 2^32 and sails
  // past a bare "greater than zero". Taking it poisons video_rate, which is
  // unsigned and wraps in every product it appears in: -64 reads as
  // 4294967232, which turns the drain poll from 418ms into 44s and leaves a
  // decoder that is already struggling no way back. Only accept durations a
  // stream could actually have.
