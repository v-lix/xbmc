...
void CAdvancedSettings::ParseAudioLatencySettings(TiXmlElement* pAudioElement)
{
  // Store global display latency settings
  TiXmlElement* pAudioLatency = pAudioElement->FirstChildElement("latency");
  if (!pAudioLatency)
    return;
  ...
  for (TiXmlElement* pPassthroughAudioLatency = pAudioLatency->FirstChildElement("passthrough");
       pPassthroughAudioLatency;
...
void CAdvancedSettings::ParseSettingsFile(const std::string &file)
{
  ...
  if (pElement)
  {
    ...
    ParseAudioLatencySettings(pElement);
  }
